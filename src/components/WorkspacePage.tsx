...
export const WorkspacePage: React.FC<WorkspacePageProps> = ({
  ...
  return (
    <div className="w-full bg-white text-slate-900 min-h-screen pb-20">
      {/* Header Bar */}
      <div className="border-b border-slate-100 bg-slate-50 py-10 px-6 sm:px-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center space-x-2 px-3 py-1 bg-indigo-50 border border-indigo-100 text-[11px] font-mono tracking-widest text-indigo-700 uppercase rounded-full mb-3">
          ...
          <div className="flex items-center space-x-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold tracking-wide uppercase transition-colors rounded shadow-sm flex items-center space-x-2"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-14">
          {GOOGLE_AI_STUDIO_GUIDE.keyBenefits.map((benefit, idx) => (
            <div
              key={idx}
              className="p-5 border border-slate-200 bg-white rounded-lg shadow-sm hover:border-slate-400 transition-colors"
            >
              <div className="w-7 h-7 rounded bg-slate-100 text-slate-900 flex items-center justify-center font-mono text-xs font-bold mb-3">
                0{idx + 1}
              </div>
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm bg-white">
          <div className="p-6 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <span className="text-xs font-mono uppercase tracking-widest text-slate-500 font-semibold block mb-1">
            ...
            <div className="p-6 flex flex-col justify-between">
              <div>
                ...
                <div className="mb-4">
                  ...
                  <pre className="p-3 bg-slate-950 text-slate-200 text-xs font-mono rounded overflow-x-auto whitespace-pre-wrap max-h-56 leading-relaxed">
                    {selectedBlueprint.systemPrompt}
                  </pre>
                </div>
              </div>
            ...
            <div className="p-6 flex flex-col justify-between bg-slate-50/50">
              <div>
                ...
                <textarea
                  ...
                  rows={9}
                  className="w-full p-3 text-xs font-mono text-slate-900 bg-white border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-slate-900 focus:border-slate-900 leading-relaxed mb-4 shadow-inner"
                  placeholder="Paste your TypeScript/Python code or algorithmic question..."
                />

                {/* Quick Execution Instructions */}
                <div className="bg-white p-3.5 border border-slate-200 rounded text-xs text-slate-600 mb-6">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 pt-4 border-t border-slate-200">
                <button
                  ...
                  className="w-full sm:w-1/2 px-4 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-800 text-xs font-semibold rounded flex items-center justify-center space-x-2 transition-colors cursor-pointer"
                >
                  {copiedState ? (
                    <>
                ...
                <button
                  ...
                  className="w-full sm:w-1/2 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded flex items-center justify-center space-x-2 transition-colors shadow-sm cursor-pointer"
                >
